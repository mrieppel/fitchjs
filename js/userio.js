...
function ckproof() {
	if(PROOF.length==0) {return errmess([0],'ERROR: No proof to check');}
	for(var i=0;i<PROOF.length;i++) {
		try {
			ckRest(PROOF[i],1);
		} catch(err) {
			return errmess([0,(i+1)],'ERROR: There is a problem with proof line '+(i+1)+'.  The error message concerning it is:<br /><br />'+err);
		}
	}
	var lastln = PROOF[PROOF.length-1];
	for(var i=0;i<lastln.sig.length;i++) {
		if(lastln.sig[i]!=1) {
			return errmess([0],'WARNING: proof is incomplete.  The final line of your proof depends on line '+lastln.sig[i]+', which is not a Premise!');
		}
	}
	if(lastln.frm!=CONCLUSION[0]) {
		return errmess([0],'WARNING: proof is incomplete.  The final line of your proof does not match the conclusion '+CONCLUSION[0]+' you are aiming for.');
	}
	return errmess([2],'Proof checks out!');
}
...
// 	var ipr = PROOF.slice(0);
// 	var ic = CONCLUSION.slice(0)[0];
// 	
// 	try{var extract = extract_proof();} catch(err) {return errmess([1],err);}
// 	var spr = extract[0];
// 	var sc = extract[1][1];
// 	
// 	PROOF = []; // need to reset PROOF because validate_line assumes that the proof to which the line to be validated belongs is stored in the global PROOF
// 	for(var i=0;i<spr.length;i++) {
// 		try {
// 			validate_line(spr[i]);
// 			PROOF.push(spr[i]);
// 		} catch(err) {
// 			alert("Validation problem. Does not Match")
// 		}
// 	}
// 	PROOF = ipr.slice(0); // set PROOF back to original internal proof
// 
// 	ipr = ipr.map(make_small);
// 	spr = spr.map(make_small);
// 	if(JSON.stringify(ipr)!=JSON.stringify(spr)) {
// 		alert("Does not match!");
// 	} else {alert("Match!");}
// 	
// 	function make_small(l) {
// 		var out = {};
// 		out.cnt = l.cnt;
// 		out.sig = l.sig;
// 		out.frm = l.from;
